...
COPY ci/texlive2025.profile ./texlive.profile

RUN \
  export INSTALL_TL_REPO=https://ftp.math.utah.edu/pub/tex/historic/systems/texlive/2025/tlnet-final && \
...
RUN \
  export TLNET_REPO=http://ftp.math.utah.edu/pub/tex/historic/systems/texlive/2025/tlnet-final ; \
  ...
  tlmgr install \
    ...
    collection-pstricks \
  ; while [ $? -ne 0 ]; do !!; done ; \
  export TLNET_REPO=https://ftp.math.utah.edu/pub/tex/historic/systems/texlive/2025/tlnet-final ; \
  tlmgr repository set ${TLNET_REPO} ; \
